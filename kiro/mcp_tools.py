...
from kiro.chinese_search import contains_chinese, search_chinese_web
from kiro.config import CHINESE_WEB_SEARCH_ENABLED
...
async def call_kiro_mcp_api(
    ...
    if CHINESE_WEB_SEARCH_ENABLED and contains_chinese(query):
        logger.info(f"WebSearch query routed to local SearXNG: {query}")
        local_results = await search_chinese_web(query)
        if local_results is not None:
            return tool_use_id, local_results
        logger.info("Continuing WebSearch through Kiro MCP fallback")
